...
long test_invoke_syscall(long syscall_number)
{
    ...
    result = syscall(syscall_number);

    if (result < 0) {
        test_fail(
            "syscall %ld fallita: %s",
            syscall_number,
            strerror(errno)
        );
    }
    ...
    __u32 syscall_number)
{
    ...
    if (name_length >= sizeof(fixture->program.name)) {
        test_fail(
            "nome programma troppo lungo: %s",
            program_name
        );
    }
